...
{   
   /**
   * @param Type reqData["title"] 邮件subject 主题
   * @param Type reqData["content"] 邮件内容
   * @param Type reqData["fromname"] 发件人姓名
   * @param Type reqData["receiver"] 收件人邮箱
   * @return boolean 发送成功会返回true，失败false
   */
    public function qqComMailSend($reqData = null)
    {
        ...
        $mail = new Lib_Smtp();
        $PHPMailer = array(
            'host' => 'smtp.exmail.qq.com',//qq邮箱使用smtp.qq.com，qq企业邮箱使用smtp.exmail.qq.com
            'username' => 'abc@abc.com',//邮箱登陆账号
            'password' => '123456',//邮箱登陆密码，也有可能是独立密码，就是开启pop3/smtp时的授权码
            'port' => 465,//默认25，腾云服务器屏蔽25端口，所以用的465
            'ssl' => true,//是否开启ssl，用465就得开启
            'from' => 'abc@abc.com'//发件人
        );
        ...
        $mail->setFrom($PHPMailer["from"]); //发送者邮箱
        $mail->setFromName($reqData["fromname"]); //发送者姓名
        $mail->setReceiver($reqData["receiver"]); //接收者邮箱
        //$mail->addAttachment(""); //Attachment 附件
        $mail->setMail($reqData["title"], $reqData["content"]); //标题和内容
        return $mail->send();
    }
}
